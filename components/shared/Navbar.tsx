    ...
    const fetchUserProfile = useCallback(async () => {
        try {
            ...
            if (!parseResult.isJson) {
                console.warn(
                    "Non-JSON response from profile:",
                    parseResult.data
                );
                return;
            }
            ...
            if (!roles || roles.length === 0) {
                try {
                    ...
                    if (testResponse.ok) {
                        const testParseResult = await safeParseResponse(
                            testResponse
                        );
                        ...
                        const testData =
                            testParseResult.data as UserProfileResponse;
                ...
                if (roles.length > 0 && typeof roles[0] === "object") {
                    roles = (roles as unknown as Role[]).map(
                        (role: Role) => role.name
                    );
                }
            }
            ...
            try {
                ...
                if (teamResponse.ok) {
                    const teamParseResult = await safeParseResponse(
                        teamResponse
                    );
