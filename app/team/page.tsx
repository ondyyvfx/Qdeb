    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                // Определяем текущего пользователя по данным из токена
                ...
                try {
                    ...
                    if (tokenPayload.sub !== undefined && tokenPayload.sub !== null) {
                        currentUsername = String(tokenPayload.sub);
                    }
                    const rawId =
                        tokenPayload.id ??
                        tokenPayload.userId ??
                    ...
                    const rawEmail = tokenPayload.email ?? tokenPayload.userEmail;
                    if (rawEmail !== undefined && rawEmail !== null) {
                        currentEmail = String(rawEmail);
                    }
                } catch (e) {
                ...
                const isLeaderById =
                    currentUserId !== null &&
                    ((teamData.leader?.id !== undefined && String(teamData.leader.id) === currentUserId) ||
                        (teamData.leaderId !== undefined && String(teamData.leaderId) === currentUserId) ||
                        (teamData.creatorId !== undefined && String(teamData.creatorId) === currentUserId));

                const isLeaderByUsername =
                    currentUsername !== null &&
                    ((teamData.leader?.username && teamData.leader.username === currentUsername) ||
                        (teamData.leaderUsername && teamData.leaderUsername === currentUsername));

                const isLeaderByEmail =
                    currentEmail !== null &&
                    ((teamData.leader?.email && teamData.leader.email === currentEmail) ||
                        (teamData.leaderEmail && teamData.leaderEmail === currentEmail));
                ...
                setTeam({
                    ...
                    leader: isLeaderById || isLeaderByUsername || isLeaderByEmail,
                });
            } catch (error) {
