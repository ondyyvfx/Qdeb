...
const TournamentDetailPage = () => {
    ...
    const fetchTournament = async () => {
        try {
            ...
            const response = await fetch(
                `${API_URL}/tournaments/${tournamentSlug}`,
                {
                    cache: "no-store",
                }
            );
            ...
            const normalized: Tournament = {
                ...
                fee:
                    typeof rawData?.fee === "number"
                        ? rawData.fee
                        : Number.parseFloat((rawData?.fee as string) ?? "0") ||
                          0,
    ...
    const loadApplications = async () => {
        ...
        if (res.status === 200 && res.data) {
            setApplications(
                res.data as unknown as TournamentApplicationListItem[]
            );
        } else {
                        ...
                        <CardContent className="p-6 md:p-10 space-y-6">
                            ...
                            {tournament.imageUrl && (
                                <div className="rounded-xl overflow-hidden border border-white/10">
                                    <img
                                        src={
                                            tournament.imageUrl.startsWith(
                                                "http"
                                            )
                                                ? tournament.imageUrl
                                ...
                                tournament.registrationFields.length > 0 && (
                                    <Card className="bg-white/5 border-white/10">
                                        ...
                                        <CardContent className="space-y-3">
                                            {tournament.registrationFields.map(
                                                (field, index) => (
                                                    <div
                                                        key={field.id ?? index}
                            ...
                            {showApply && (
                                <Card className="bg-white/5 border-white/10">
                                    <CardHeader>
                                        ...
                                        <CardDescription>
                                            Заполните форму и отправьте на
                                            рассмотрение организатору
                                        </CardDescription>
                                    </CardHeader>
                                    <CardContent className="space-y-4">
                                        {!user && (
                                        ...
                                        {user && !team && (
                                            <p className="text-sm text-amber-400">
                                                ...
                                                Создайте или вступите в команду.
                                            </p>
                                        )}
                                        {user &&
                                            team &&
                                            team.memberCount !== 2 && (
                                                <p className="text-sm text-amber-400">
                                                    Заявку может подать только
                                                    команда из 2 участников.
                                                </p>
                                            )}
                                        {user && team && (
                                            ...
                                            {hasHydrated &&
                                                !user?.roles?.includes(
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={() =>
                                                            router.push(
                                                                `/tournaments/${tournament.slug}/join`
                                                            )
                                                        }
                                                        disabled={!user}
                                                    >
                                    ...
                                    {user?.roles?.includes("ROLE_ADMIN") && (
                                        <div className="space-y-2 pt-2 border-t border-white/10">
                                            <Button
                                                ...
                                                onClick={() =>
                                                    router.push(
                                                        `/tournaments/${tournamentSlug}/edit`
                                                    )
                                                }
                                            >
