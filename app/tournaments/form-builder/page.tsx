...
const createField = (type: RegistrationFieldType = "TEXT"): RegistrationField => ({
  id: `field-${generateId()}`,
  name: "Новое поле",
  title: "Новое поле",
  description: "",
  ...
  scale: type === "linear_scale" ? { min: 1, max: 5, minLabel: "Минимум", maxLabel: "Максимум" } : undefined,
});
  ...
  try {
    ...
    const normalized = parsed
      .map((item: unknown) => {
        ...
        return {
          id: (typeof field?.id === "string" ? field.id : `field-${generateId()}`) as string,
...
export default function TournamentFormBuilderPage() {
  ...
  const handleTypeChange = (fieldId: string, nextType: RegistrationFieldType) => {
    setFields((prev) =>
      prev.map((field) => {
                  ...
                  return (
                    <Card
                      ...
                      className={`overflow-hidden border-2 transition-all duration-200 ${
                        isActive ? "border-accent shadow-lg" : "border-white/10"
                      }`}
                    >
                      <CardHeader className="flex flex-col gap-4 bg-white/3 backdrop-blur-sm sm:flex-row sm:items-start sm:justify-between">
                        <div className="flex flex-1 flex-col gap-3">
                          ...
                          <Input
                            value={field.title}
                            onChange={(event) =>
                              setField(field.id, { title: event.target.value, name: event.target.value })
                            }
                          ...
                          <Textarea
                            ...
                            onChange={(event) =>
                              setField(field.id, { description: event.target.value })
                            }
                        ...
                        <div className="flex max-w-[220px] flex-col gap-3">
                          ...
                          <select
                            value={field.type}
                            onChange={(event) =>
                              handleTypeChange(field.id, event.target.value as RegistrationFieldType)
                            }
                          ...
                          <div className="flex items-center justify-between rounded-md border border-white/10 bg-white/5 px-3 py-2">
                            <span className="text-xs font-medium text-white/70">Обязательное</span>
                            <Checkbox
                      ...
                      <CardContent className="space-y-4 bg-background/80">
                            ...
                            {field.options.map((option, optionIndex) => (
                              ...
                              >
                                ...
                                <Input
                                  value={option}
                                  onChange={(event) =>
                                    handleOptionChange(field.id, optionIndex, event.target.value)
                                  }
                          ...
                          <div className="space-y-4 rounded-lg border border-white/10 bg-white/5 p-4">
                            <div className="grid gap-4 sm:grid-cols-2">
                              <div className="space-y-2">
                                ...
                                <Input
                                  ...
                                  onChange={(event) =>
                                    handleScaleChange(field.id, "min", event.target.value)
                                  }
                              ...
                              <div className="space-y-2">
                                ...
                                <Input
                                  ...
                                  onChange={(event) =>
                                    handleScaleChange(field.id, "max", event.target.value)
                                  }
                                  className="border-none bg-background/60 text-white focus-visible:ring-2 focus-visible:ring-accent"
                                />
                              </div>
                            ...
                            <div className="grid gap-4 sm:grid-cols-2">
                              <div className="space-y-2">
                                ...
                                <Input
                                  value={field.scale?.minLabel ?? "Минимум"}
                                  onChange={(event) =>
                                    handleScaleChange(field.id, "minLabel", event.target.value)
                                  }
                                  className="border-none bg-background/60 text-white focus-visible:ring-2 focus-visible:ring-accent"
                                />
                              </div>
                              <div className="space-y-2">
                                ...
                                <Input
                                  value={field.scale?.maxLabel ?? "Максимум"}
                                  onChange={(event) =>
                                    handleScaleChange(field.id, "maxLabel", event.target.value)
                                  }
                                  className="border-none bg-background/60 text-white focus-visible:ring-2 focus-visible:ring-accent"
                                />
                              </div>
                        ...
                        {(field.type === "DESCRIPTION" || field.type === "paragraph") && (
                          <div className="rounded-lg border border-dashed border-white/10 bg-white/5 px-4 py-3 text-sm text-white/60">
              ...
              <div className="space-y-4">
                <Card className="border-white/10 bg-white/5 backdrop-blur">
                  ...
                  <CardContent className="space-y-3 text-sm text-white/80">
                    <div>
                      <p className="text-base font-semibold text-white">Форма регистрации на турнир</p>
                      <p className="text-xs text-white/60">Заполните все поля для подачи заявки</p>
                    </div>
                    <ul className="space-y-3">
                      {fields.map((field, index) => (
                        <li key={`preview-${field.id}`} className="rounded-lg border border-white/10 bg-background/60 p-3">
                          ...
                          <div className="mt-3 text-xs text-white/50">
                            {field.type === "TEXT" && "Короткий ответ"}
                            {field.type === "DESCRIPTION" && "Развернутый ответ"}
                            {field.type === "short_answer" && "Короткий ответ"}
                            {field.type === "paragraph" && "Развернутый ответ"}
                            {field.type === "multiple_choice" && `Один вариант из ${field.options.length}`}
                            {field.type === "checkboxes" && `Несколько вариантов из ${field.options.length}`}
                            {field.type === "dropdown" && `Выпадающий список (${field.options.length})`}
                            {field.type === "linear_scale" &&
                              `Шкала от ${field.scale?.min ?? 1} до ${field.scale?.max ?? 5}`}
                          </div>
                        </li>
                      ))}
                    </ul>
                ...
                <Card className="border-white/10 bg-white/5 backdrop-blur">
                  ...
                  <CardContent className="space-y-3 text-sm text-white/70">
                    <p>
                      Этот конструктор позволяет создавать сложные формы регистрации для турниров. 
                      Добавляйте различные типы полей, настраивайте обязательные поля и создавайте 
                      интерактивные элементы для сбора информации от команд.
                    </p>
                    <ul className="list-disc pl-5 space-y-1">
                      <li>Используйте разные типы полей для точного сбора данных</li>
                      <li>Настраивайте обязательные поля для важной информации</li>
                      <li>В правой колонке видно, как форму увидят участники</li>
                      <li>Сохраняйте шаблоны для повторного использования</li>
                    </ul>
                  </CardContent>
                </Card>
              </div>
